...
namespace nkast.Aether.Physics2D.Diagnostics
{
    ...
    public class DebugView : DebugViewBase, IDisposable
    {
        ...
        public void DrawArrow(Vector2 start, Vector2 end, float length, float width, bool drawStartIndicator, Color color)
        {
            ...
            Microsoft.Xna.Framework.Vector2[] verts = new Microsoft.Xna.Framework.Vector2[3];
            verts[0] = new Microsoft.Xna.Framework.Vector2(0, 0);
            verts[1] = new Microsoft.Xna.Framework.Vector2(-halfWidth, -length);
            verts[2] = new Microsoft.Xna.Framework.Vector2(halfWidth, -length);

            // Rotate end shape
            Microsoft.Xna.Framework.Vector2.Transform(verts, ref rotMatrix, verts);
            // Translate end shape
            Microsoft.Xna.Framework.Vector2.Transform(verts, ref endMatrix, verts);
                       
            // Draw arrow end shape
            DrawSolidPolygon(verts.Select(x => new Vector2(x.X, x.Y)).ToArray(), 3, color, false);
            ...
            if (drawStartIndicator)
            {
                ...
                Microsoft.Xna.Framework.Vector2[] baseVerts = new Microsoft.Xna.Framework.Vector2[4];
                baseVerts[0] = new Microsoft.Xna.Framework.Vector2(-halfWidth, length / 4);
                baseVerts[1] = new Microsoft.Xna.Framework.Vector2(halfWidth, length / 4);
                baseVerts[2] = new Microsoft.Xna.Framework.Vector2(halfWidth, 0);
                baseVerts[3] = new Microsoft.Xna.Framework.Vector2(-halfWidth, 0);

                // Rotate start shape
                Microsoft.Xna.Framework.Vector2.Transform(baseVerts, ref rotMatrix, baseVerts);
                // Translate start shape
                Microsoft.Xna.Framework.Vector2.Transform(baseVerts, ref startMatrix, baseVerts);
                // Draw start shape
                DrawSolidPolygon(baseVerts.Select(x => new Vector2(x.X, x.Y)).ToArray(), 4, color, false);
            }
        }
                                    ...
                                    BlendState blendState = null, SamplerState samplerState = null, DepthStencilState depthStencilState = null, RasterizerState rasterizerState = null, float alpha = 1.0f)
        {
            ...
            for (int i = 0; i < _stringData.Count; i++)
            {
                if (_stringData[i].Text != null)
                    _batch.DrawString(_font, _stringData[i].Text, _stringData[i].Position.ToXna(), _stringData[i].Color);
                else
                    _batch.DrawString(_font, _stringData[i].stringBuilderText, _stringData[i].Position.ToXna(), _stringData[i].Color);
            }
        ...
        public void LoadContent(GraphicsDevice device, IPrimitiveBatch primitiveBatch = null)
        {
            ...
            _font = AssetManager.Load<SpriteFont>("DiagnosticsFont");
